...
| `CaptionPlacer` (pure): never covers anything. For each message it tries, in order: under it; beside it in the empty space next to the bubble; a compact style beside it; a compact style under it, and takes the first place where the whole translation fits (else the first with at least one line, ended with an ellipsis, else no caption). Every other message, name or quote is an obstacle, and so is each earlier caption by the space it actually uses (measured line count, not its maximum). Stays inside the area above the text box | IMPLEMENTED — 15 unit tests, including a run of five consecutive bubbles from the owner's phone |
