...
| `CaptionPlacer` (pure): never covers anything. For each message, in order: under it (only if there is room before the next message, name, quote or the text box), beside it in the empty space next to the bubble, a compact one-line caption under it, otherwise no caption. Earlier captions count as obstacles too, so captions never overlap each other. Added after the owner saw the first version cover following messages on a real chat | IMPLEMENTED — 13 unit tests, including tight consecutive bubbles |
