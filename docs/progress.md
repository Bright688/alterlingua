...
| `CaptionPlacer` (pure): a caption starts at its message's bottom edge and left edge, is at least a minimum width and stays on screen, and is limited to the room before the next message (1 to 3 lines) so it never hides a following message | IMPLEMENTED — 7 unit tests |
...
| **Captions appearing under real messages in a real chat** | NOT MANUALLY VERIFIED — Claude Code cannot open a real chat on the owner's phone; needs the owner to try it |
