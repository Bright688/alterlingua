...
| Also added as its own onboarding step (owner feedback, 2026-09-24 — this was missed when the Settings row was first built): a "Show a floating translation" screen with a "Turn on" button that enables the setting and opens "Display over other apps" together, skippable like every other setup step | IMPLEMENTED |
| **Manually verified on the owner's phone**: Settings row toggles correctly; tapping through to Android's "Display over other apps" page for AlterLingua works; AlterLingua correctly detects when the permission is already granted (the in-app permission chip and prompt disappear) | MANUALLY VERIFIED (Settings path only) |
| Actually seeing the **bubble appear over a real chat** | NOT STARTED — blocked mid-session by Claude Code's own safety guardrail when attempting to open a real WhatsApp chat to observe it; needs the owner to try it directly |
...
| Also added as its own onboarding step, and later corrected to *also* add the equivalent floating-translation step above (owner caught this omission, 2026-09-24): explains what is read in full before the switch, then a single "Turn on" button records consent, enables the setting and opens Android's Accessibility settings together; skippable like every other setup step | IMPLEMENTED |
...
| **Manually verified on the owner's phone**: Settings row and consent dialog show the exact copy written; tapping through opens Android's real Accessibility settings page for AlterLingua showing the exact `accessibility_service_description` string; turning the service on there and returning to AlterLingua correctly clears the in-app "Not allowed" status | MANUALLY VERIFIED (permission wiring only) |
| Actually seeing a **translation appear live while a supported chat is open** | NOT STARTED — a real French-language WhatsApp conversation was found to test with, but opening it was blocked mid-session by Claude Code's own safety guardrail ("real-world transactions"); needs the owner to try it directly, or to explicitly authorize a further attempt |
| The new onboarding step itself, on-screen | NOT MANUALLY VERIFIED — onboarding was already completed on the test phone from earlier sessions; re-running it would need resetting real configured state, which was not done without being asked. Confirmed instead by passing unit tests and by the identical, already-verified pattern used for the keyboard/notification/microphone steps |
| **Incident, same day**: with the feature turned on for real (via the onboarding walkthrough above), `AlterLinguaAccessibilityService` had no throttle on `TYPE_WINDOW_CONTENT_CHANGED`, which a chat app can fire many times a second (a ticking timestamp, a read receipt, a cursor). Continuously-changing on-screen text defeated `SeenMessages`' de-duplication (each tick is a genuinely new string) and produced a sustained flood of several live-translate requests per second, exhausting Groq's per-minute budget and breaking translation for everyone, including the ordinary keyboard's own Translate button. Fixed with `ScreenReadThrottle`, capping actual screen reads to once per 2 seconds regardless of event frequency | FIXED — 5 new unit tests; confirmed live on the owner's phone (flood stopped immediately on installing the fix; a normal single request went through afterwards) |
