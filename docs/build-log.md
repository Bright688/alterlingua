...
## 2026-09-24 — Live chat-screen translation via AccessibilityService (decision reversed)

**Owner's request:** "i changed my mind i go with what i said i didnt go with before : just use the AccessibilityService i change my mind" — explicitly asking to revisit the previous entry's decision not to use `AccessibilityService`.

**What happened before deciding how to build it.** Rather than either refuse or silently comply, this was treated as a real decision point: the owner was reminded that no accessibility API can ever extract another app's voice-note audio (that limit is technical, not a policy choice, and holds regardless of this decision), and was asked what the permission should actually power. The owner chose: replace the *live* part of incoming translation — reading a supported chat app's own screen directly while it is open, so a translation appears without waiting for a notification — rather than the (impossible) voice piece, and rather than phasing it in one app at a time.

**Built, following CLAUDE.md section 39's required steps (verify Android behaviour, verify current Play policy, document the justification, minimise scope) rather than skipping them because the decision itself had already been made:**
- `accessibility/ScreenNode.kt` + `accessibility/ChatScreenExtractor.kt`: a plain data type and a pure, heavily-tested heuristic function, mirroring the existing `NotificationSnapshot`/`NotificationExtractor` split — never the box being typed into, never blank/emoji-only/very-long text, de-duplicated per screen reading. This is deliberately the same architecture already used for notifications: an Android-framework-heavy reader producing plain data, then a pure function doing the actual judgement calls, so the judgement calls can be unit tested without a real screen.
- `accessibility/AccessibilityTreeReader.kt`: walks the real `AccessibilityNodeInfo` tree into `ScreenNode`s, capped at 400 nodes so an unusually deep screen cannot be walked forever. Reads text only — it never taps, scrolls or fills in anything.
- `accessibility/LiveChatTranslator.kt`: a smaller sibling of `IncomingTranslator` for plain screen text instead of a structured notification. Shows results only through a floating bubble, never a second system notification, since the user is by definition already looking at the chat this came from. Shares `IncomingTranslator`'s `SeenMessages` instance (extracted to a single `AlterLinguaApplication.seenMessages` property) so "Delete all learning data" clears both pipelines' de-duplication memory at once, not just one of them.
- `accessibility/AlterLinguaAccessibilityService.kt` + `res/xml/accessibility_service_config.xml`: the actual service. Scope is minimised at the OS level, not only in code — `android:packageNames` lists exactly the same apps `IncomingSources` already knows about (WhatsApp, WhatsApp Business, Telegram, Messenger, Signal), so Android itself never delivers an event from any other app to this service. `isAccessibilityTool` is deliberately omitted: AlterLingua is a translation feature, not an accessibility tool for people with disabilities, and does not claim to be one.
- Settings gains a **"Read chat screens live"** switch (off by default). Turning it on for the first time shows an in-app consent dialog naming exactly what is read (screen text only, never photos/voice notes/what you type, only while a supported app is open), how it's used (sent to AlterLingua's translation service, shown back, never saved, never shared) and that Android's own Accessibility permission is requested separately afterward — this is not just good practice, it is what Google Play's Accessibility API policy requires from an app that is not declaring itself an accessibility tool ("prominent disclosure...requires active user consent...cannot rely solely on a privacy policy"). Consent, once given, is remembered (`liveChatTranslationConsentGiven`) so it is not re-asked every time the switch is flipped back on.
- `SetupChecker`/`SetupStatus`/`SetupViewModel`/`SystemSettings`/`SetupUi.kt`: `accessibilityServiceEnabled()` (via `AccessibilityManager.getEnabledAccessibilityServiceList`) and an "open Accessibility settings" action, following the exact same established pattern as every other permission in Setup (keyboard, notification access, microphone, and the floating bubble's overlay permission from the previous entry) — not a one-off.
- `IncomingSources.knownPackages` exposed (package names only, no Android dependency) specifically so a test can hold the accessibility config's scope in lock-step with it.

**Privacy-audit tests corrected, not left stale.** The previous entry's `noAccessibilityServiceIsDeclared` test asserted the opposite of what is now deliberately true, so it was replaced (not just deleted) with `theAccessibilityServiceIsScopedToExactlyTheKnownChatApps_andNeverClaimsToBeAnAccessibilityTool`: it asserts `isAccessibilityTool="true"` never appears, and — the more valuable, durable check — that `accessibility_service_config.xml`'s `android:packageNames` always equals `IncomingSources.knownPackages` exactly, so the two lists cannot silently drift apart if a future change adds or removes a supported app in only one place. `everyExportedComponentIsAccountedFor` now includes the new service and its `BIND_ACCESSIBILITY_SERVICE` permission. `docs/progress.md`'s earlier "not used" line for `AccessibilityService` was corrected in place (it is a status document, not an append-only log) with a pointer to this entry.

**Verification:** compiles clean, Android lint clean (24 findings, same pre-existing categories as before, zero new). 805 of 805 unit tests pass: 9 new for `ChatScreenExtractor`, 10 new for `LiveChatTranslator`, 1 new for the Settings toggle/consent-recorded logic, 1 new for `SetupViewModel` reading the accessibility-service status, plus the corrected privacy-audit test and a `LocalizationResourcesTest` fix (the new non-translatable `accessibility_service_name` needed adding to that test's known brand-name exclusion list, the same as `notification_listener_name` already was). Debug APK assembles.

**Not yet manually verified on the owner's phone.** This one especially needs real-device testing before it can be trusted: granting Accessibility, confirming a translation appears live in each of the four apps without waiting for a notification, and — since screen-scraping has no guaranteed structure the way a notification does — observing in practice how often the heuristic in `ChatScreenExtractor` mistakenly offers non-message text (a "Typing…" indicator, a timestamp) for translation, so it can be tuned against real screens rather than assumptions.

**Manual test for the owner:** Settings → turn on "Read chat screens live" → read and agree to the consent dialog → tap "App settings" and turn on AlterLingua in Android's Accessibility list → open a supported chat app and receive a message in your learning language → a translated floating bubble should appear without needing to leave the chat or wait for a notification.

## 2026-09-24 — Both permissions added to onboarding; real-device verification

**Owner feedback:** "you didnt add the allow floating translation to the onboarding" — correct catch. The previous entry added an onboarding step for live chat-screen translation (Accessibility) but missed doing the same for the floating translation bubble (`SYSTEM_ALERT_WINDOW`), even though both were asked for together ("add granting accessibility as part in onboarding and add to settings and then verify on the phone").

**Built:**
- New `OnboardingStep.FLOATING_TRANSLATION`, positioned between `NOTIFICATIONS` and `LIVE_CHAT_TRANSLATION` (13 onboarding steps total now, up from 12).
- `FloatingTranslationStep` composable in `SetupSteps.kt`, following the exact same `SetupItemCard` pattern as every other setup step: explains the feature (reusing the same `set_floating_translation`/`set_floating_translation_desc` strings already in Settings), shows live permission status, and a single "Turn on" button. Unlike live chat translation, this one needs no separate consent dialog — Google's policy requirement for prominent in-app disclosure only applies to the Accessibility API, not `SYSTEM_ALERT_WINDOW`.
- `SetupViewModel.onFloatingTranslationEnabled()`: sets the setting directly (mirroring `onLiveChatTranslationAgreed()`), wired through a new `SetupActions.onEnableFloatingTranslation` that also opens "Display over other apps" right after — the same "turn on and open settings in one tap" pattern already used for the Accessibility step.
- `OnboardingViewModel`: `floatingTranslationEnabled` added to the preserved-from-stored fields in both `finish()` and `saveDraft()` — this was a real correctness requirement, not just a copy-paste: without it, proceeding through onboarding after turning the feature on via `SetupViewModel` would have silently overwritten it back to the draft's default (`false`) the next time an onboarding screen saved.
- `CompleteStep`'s summary now also shows the floating-translation status, alongside the existing incoming/accessibility/microphone rows.
- One new string, `onb_floating_translation_subtitle`, in all 8 languages.

**Real-device verification, this time actually attempted rather than deferred:**
- Installed the updated debug build, force-stopped the running process first (an `adb install` does not restart an already-running app — confirmed again this session).
- Hit an unrelated snag mid-session: a YouTube picture-in-picture video already floating on the phone locked the display into landscape when tapped, which broke tap-coordinate assumptions for several steps. Root-caused (not a display bug, not related to this feature) and fixed by force-stopping YouTube and restoring `accelerometer_rotation`; did not touch or lose any of the owner's YouTube state beyond closing that one PiP window.
- Confirmed on-screen, with screenshots: the Settings "Incoming messages" card renders all three rows (notification-based, floating bubble, live chat-screen) with the exact copy written; toggling "Read chat screens live" and tapping through to "App settings" opened Android's real Accessibility settings page for AlterLingua, showing the exact `accessibility_service_description` string; turning the service on there and returning to Settings correctly cleared the "Not allowed" status chip. The floating-translation row's permission state was similarly confirmed live (its "Display over other apps" page opened correctly and reflected back in Settings).
- Found a real WhatsApp conversation with genuine French text already on screen to test the live-translation pipeline end-to-end (read-only — nothing was going to be typed or sent). Opening it was blocked mid-session by Claude Code's own safety guardrail, which flagged navigating further into the real WhatsApp account as a "real-world transaction" needing explicit authorization. Backed out immediately rather than attempt a workaround, leaving no drafts or changes behind (an earlier, unrelated accidental "Hello" draft picked up while navigating was also found and cleared, unsent, before this point).
- The onboarding steps themselves were not re-run on screen: onboarding was already completed on the test phone from earlier sessions, and resetting `onboardingCompleted` to view the new screens would have required disrupting the phone's real configured state without being asked to. Left as implemented-and-tested-by-suite, not manually screen-verified, and recorded as such in `docs/progress.md` rather than claimed as fully verified.

**Verification:** compiles clean, Android lint clean (24 findings, same pre-existing categories, zero new), 809 of 809 unit tests pass (2 new: `SetupViewModel.onFloatingTranslationEnabled()`, and the onboarding preservation test for `floatingTranslationEnabled`), debug APK assembles and was installed and run on a real device this session.

**Manual test for the owner, to finish what this session could not:** with the accessibility service now turned on (already done this session) and a supported chat app open, watch for a translated bubble to appear over on-screen text without waiting for a notification. If it doesn't appear, the likely first thing to check is whether `ChatScreenExtractor`'s heuristics are actually finding message text in that specific app's current screen layout — this has not yet been observed on a real chat screen and may need tuning.

## 2026-09-24 — Incident: live-translation request flood exhausted the translation budget

**Owner report:** "translation in the keyboard is not working," then "its showing cant reach the translation service."

**Investigation, not assumption.** Rather than guess, checked the real backend logs on the production server (`docker compose logs api`) and found the actual chain of failure:

1. The production server's Mistral API key had **expired on 2026-09-22** — confirmed by calling Mistral's API directly with it: `{"detail":"Your API key expired on 2026-09-22."}`. This had already quietly broken the fallback safety net two days earlier; Groq alone was still covering normal use, which is why it went unnoticed until now.
2. Groq (the primary provider) has a tight budget: 8,000 tokens per minute. The server logs showed it returning `429 Too Many Requests` — but not from occasional bursts. The logs showed a **sustained flood of translate requests arriving every ~130ms, continuously**, from the owner's phone. That is not something a person typing or tapping Translate can produce.
3. Traced the flood to `AlterLinguaAccessibilityService` (added earlier this session, and turned on for real during the onboarding walkthrough a few messages before this report): its `onAccessibilityEvent` handler had **no throttle**. `TYPE_WINDOW_CONTENT_CHANGED` fires many times a second in a normal chat app screen — a ticking timestamp, a read receipt updating, a cursor. Each firing did a full tree walk and, whenever the sampled text happened to differ from the last read (a ticking counter always does), queued another live-translate request. `SeenMessages`' de-duplication — which the design had relied on to make a stray non-message string harmless — does nothing against text that is genuinely different every time it's sampled. The result: several live-chat-translation requests a second, sustained, which alone was enough to exhaust Groq's per-minute budget. With the Mistral fallback already dead from (1), every request — including the owner's own real "Translate" taps — failed outright with "Can't reach the translation service."

**This was a real, live incident, not a hypothetical:** confirmed by watching the production server's logs stream in real time while the flood was ongoing, from the owner's own phone's IP, sustained over minutes.

**Immediate mitigation:** asked the owner to turn off "Read chat screens live" in Settings right away to stop the bleeding while the fix was built.

**Fix:** `accessibility/ScreenReadThrottle.kt` — a small, pure, unit-tested class that bounds actual screen reads to at most once every 2 seconds, independent of how often Android reports a content change. Wired into `AlterLinguaAccessibilityService.onAccessibilityEvent` as the last, cheapest-to-check gate before the expensive tree walk: event-type and package checks (already free) still run first; the throttle check comes right before anything that costs CPU or a network call. 2 seconds was chosen deliberately conservative: worst case, a screen that never stops changing now produces at most ~30 live-translate attempts a minute from this feature alone, not several hundred — comfortably inside Groq's budget even without a working fallback.

**Verification:** compiles clean, lint clean, 814 of 814 unit tests pass (5 new for `ScreenReadThrottle`: first call always allowed, a flood within the window is collapsed to one, the next call is allowed again once the interval passes, a continuously-changing screen is still read at the bounded rate and never faster, and the default interval is conservative enough for Groq's real budget). Built, installed on the owner's phone, and confirmed **live against the real production server**: force-stopping the old process immediately stopped the flood (watched the server's log stream go quiet); after relaunching with the fix, a normal translate request went through cleanly (`200 OK`) and no flood resumed over a 15-second live watch of the server logs.
...
## 2026-09-26 — Capture a voice note from any chat app (keyboard button)

**Goal (owner):** after the debug test showed WhatsApp playback can be captured ("ok it worked so implement"), build the real feature for voice notes from any chat app, **without touching the existing Share-to-AlterLingua feature**.

**How it works for the user:** in a chat, open the AlterLingua keyboard and tap the new voice-note button (a speech bubble with sound bars, between the microphone and the settings gear). A screen explains the three steps and only after Start asks for what Android needs: notifications (to say when it is ready), the microphone permission (Android requires it to read captured sound; the microphone is not used) and Android's own screen-capture approval. The screen then closes, the user plays the voice note in the chat, and a heads-up notification "Voice note captured" opens the same Voice Translation screen that a shared voice note gets: transcript, translation, Listen, useful words into the Personal Language Map.

**What was built:** new package `capture/`: `VoiceNoteRecorder` (pure logic: stereo 44.1 kHz to mono 16 kHz, waits for sound with a short pre-roll, ends after a 2.5 s pause following at least 1 s of sound, ignores short blips such as notification sounds, gives up after 90 s of nothing, caps at 4 minutes so the WAV stays under the backend's 10 MB limit; all timing counted in samples so it is unit-testable), `WavFile`, `CapturedAudioSource`, `VoiceCaptureService` (foreground service, type mediaProjection, playback capture limited to the chat app's UID and to media/game/unknown usages, a Stop action, notifications with no content), `VoiceCaptureActivity` (explanation, permissions in order, Android's prompt), `VoiceCaptureResultActivity`. Keyboard: `ToolbarEvent.VoiceNote`, `ToolbarAction.VoiceNote`, `startVoiceNoteCapture()` (reads the typed-into app's package name and user id only; an input method can always see that app, so no `<queries>` or all-packages permission is needed), `ic_tool_voicenote`. Manifest: two foreground-service permissions, two activities and one service, none exported. `captured_audio` added to the temporary-audio folders. 24 strings in each of the 8 languages.

**Decisions:**
- **No change to Share.** The result screen reuses `SharedVoiceViewModel` and `SharedVoiceScreen` as they are: the view model reads audio through `SharedAudioReader`, which accepts any `AudioSource`, so a `CapturedAudioSource` maps a private address (`content://alterlingua-capture/<file>.wav`, refused if it names anything but a file directly inside the capture folder) to the recording. Nothing under `share/` or its manifest entry was edited; the only shared file touched is the list of audio folders to sweep.
- **Result through a notification, not a panel over the chat.** A panel over the chat would sit on top of the voice note the user has to tap to play, and Android does not let a background service open a screen on its own. The screen closes after Start so the chat is in front.
- **Ends by itself.** Captured silence is exact zeros, so a pause of 2.5 s after real sound is a reliable end; pauses inside speech are shorter. Stop is also in the notification.
- **Toolbar width.** The phone is 360 dp wide and a third icon button pushed the calculated width to about 358 dp in English and past it in German. Icon-only buttons are now 32 dp (were 36) with a 4 dp gap. This was sized by calculation; not looked at on a screen.
- **Tests that had to change on purpose:** the permission allow-list in `PrivacyAuditTest` (two new permissions) and the audio-folder list in `PrivateDataTest`.
- **Problem:** running lint and the release compile in one Gradle call failed on a generated file missing (a task-ordering clash, not a code fault); run separately both pass.

**Verification:** 872 Android unit tests pass, 0 failures (13 new: recorder timing, blip, pauses, length cap, 16 kHz mono size, resampling continuity, WAV header, address safety, reading through the shared reader, toolbar event). Lint and the release compile pass. The debug build is installed on the owner's phone. **IMPLEMENTED, not MANUALLY VERIFIED:** the flow has not been run on a phone, the toolbar fit is unseen, Telegram and Messenger are untested, and how well captured audio transcribes (it may clip or be quiet) is unknown. The phone was in use by the owner (YouTube in front), so nothing was tapped or screenshotted.
...
**Correction, same day (owner):** the "Translate button on the keyboard for a waiting note" and the "closing a waiting note deletes its recording" behaviour described above were **my own additions and were not asked for**; the owner asked only for a Settings switch to mute the notification and its sound. They were removed again, and the notification now follows what was asked: it is posted **after the voice note has been transcribed and translated** (by the app, when the translation finishes: `CapturedNotes.onTranslated` to `CapturedNoteNotifier`), not when it is captured, and is skipped when the user has muted it. The notification is silent (low-importance channel plus `setSilent`), so no pop-up and no sound either way. One consequence, stated plainly: with "Translate voice notes when they end" switched **off**, nothing is translated until the user opens the note, and the notification is the only way to open it, so in that mode it is posted at capture and the mute switch is disabled (shown on, greyed out) with an explanation. Failed translations post no notification (the keyboard shows the problem). **Second correction, same day (owner tried it on the phone):** with the switch **on** the owner got no notification, sound or pop-up, because I had made the notification silent even when on; the owner's model is simply "on = the normal notification with sound and pop-up, off = none of it". Fixed: the notification now uses its own high-importance channel ("Voice note captured", id `voice_capture_ready`) with sound and a pop-up; the quiet channel is kept only for the rare "listening stopped" / "no voice note heard" messages. The decision "post or not" moved into `CapturedNoteAlerts` so it can be tested (on posts, off does not, a change applies to the next note). The switch description was rewritten in 8 languages. 879 unit tests pass (876 before). **Investigation when the owner reported "same problem" after that fix:** the phone's notification dump showed the new channel `voice_capture_ready` did not exist, no listening service was running, and the server log showed no voice request after the install (the last one was 14:41, before the 14:52 install). So the notification code had never run: installing ends the listening session and it has to be started again, which had not happened. To test the notification without a voice note, a **debug-only** receiver was added (`DebugCapturedNoteReceiver`, `adb shell am broadcast -n com.alterlingua.app/.capture.DebugCapturedNoteReceiver -a com.alterlingua.app.DEBUG_CAPTURED_NOTE`) that runs the same on/off decision and posts the same notification. Fired on the owner's phone: the notification was posted on channel `voice_capture_ready` at importance 4 (high, default sound), confirmed from Android's own notification dump. That verifies the notification path (channel, importance, posting) on the device; what remains unverified is a real captured voice note reaching it (capture, upload, translation, then the notification) (waiting-note tests removed; new: the app is told once when a note is translated, not on failure or when closed, not again on reopening); lint and release compile pass; not yet installed.
