"""Translation with a Mistral chat model (https://docs.mistral.ai/api/endpoint/chat).

The model is asked for a small JSON answer: the translation and the language of the original. The user's message is passed as
data inside the user turn; the instructions live in the system turn and tell the model never to follow instructions found in the
message. Nothing is logged. The provider states the eight catalogue languages and that it can detect the source language.
"""
...
class MistralTranslationProvider(TranslationProvider):
    ...
    def __init__(self, settings: Settings, *, client: MistralClient | None = None) -> None:
        ...
        self._model = settings.mistral_translation_model
