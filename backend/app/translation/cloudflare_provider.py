"""Translation with a Cloudflare Workers AI chat model, via its OpenAI-compatible endpoint.

Same contract as the Groq and Mistral providers: the model is asked for a small JSON answer (the translation and the language of
the original), the user's message is passed as data inside the user turn, and the instructions live in the system turn and tell
the model never to follow instructions found in the message. Nothing is logged. The provider states the eight catalogue
languages and that it can detect the source language.

Default model is Qwen3-30B-A3B (FP8), Cloudflare's own model id "@cf/qwen/qwen3-30b-a3b-fp8".
"""
...
class CloudflareTranslationProvider(TranslationProvider):
    name = "cloudflare"
    ...
    def __init__(self, settings: Settings, *, client: CloudflareClient | None = None) -> None:
        ...
        self._model = settings.cloudflare_translation_model
